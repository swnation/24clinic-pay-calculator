...
// --- Personal Rate Editor ---
function PersonalRateEditor() {
  const { state, currentUser, updateMyRates } = useAppStore();
  const personalRates = currentUser?.personalRates || {};

  const handleChange = (key: keyof RatesBySlot, value: string) => {
    if (value === '') {
      // Remove personal override - revert to branch default
      const updated = { ...personalRates };
      delete updated[key];
      updateMyRates(updated as Partial<RatesBySlot>);
    } else {
      const num = Number(value);
      if (isNaN(num) || num < 0) return;
      updateMyRates({ [key]: num });
    }
  };

  return (
    <section className="bg-white rounded-lg border border-gray-200 p-6">
      <h3 className="font-bold text-lg mb-2">내 시급 설정</h3>
      <p className="text-sm text-gray-500 mb-4">
        계약 조건이 다른 경우 개인 시급을 설정하세요. 빈칸은 기본값({' '}
        <span className="text-gray-400">지점 기본 시급</span>) 적용.
      </p>
            ...
            {DAY_TYPES.map(dt => (
              <tr key={dt} className="border-b last:border-b-0">
                ...
                {TIME_SLOTS.map(ts => {
                  ...
                  const personalVal = personalRates[key];
                  const branchVal = state.defaultRates[key];
                  return (
                    <td key={ts} className="py-1.5 px-1">
                      <input
                        ...
                        min="0"
                        step="0.5"
                        value={personalVal !== undefined ? personalVal : ''}
                        placeholder={String(branchVal)}
                        onChange={e => handleChange(key, e.target.value)}
                        className={`w-full border rounded px-2 py-1.5 text-sm text-center min-w-[60px] ${
                          personalVal !== undefined ? 'border-blue-400 bg-blue-50' : ''
                        }`}
                      />
                    </td>
                  );
...
function UserManagement() {
  ...
  return (
    <section className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Google 계정과 의사 이름 연결 현황. 잘못 연결된 경우 해제할 수 있습니다.
      </p>

      <div className="space-y-2 mb-4">
        {allUsers.map(({ uid, profile }) => (
          <div key={uid} className="flex items-center gap-3 py-2 px-3 border rounded-lg">
            <span className="px-2 py-0.5 rounded text-xs font-medium" style={{ backgroundColor: getDoctorColor(profile.doctorId) }}>
              {getDoctorName(profile.doctorId)}
            </span>
      ...
      {/* Unlinked doctors */}
      {(() => {
        const linkedDoctorIds = new Set(allUsers.map(u => u.profile.doctorId));
        const unlinked = activeDoctors.filter(d => !linkedDoctorIds.has(d.id));
        if (unlinked.length === 0) return null;
        return (
          <div className="text-xs text-gray-400 mt-3">
            <p className="mb-1">미연결 의사:</p>
            <div className="flex flex-wrap gap-1">
              {unlinked.map(d => (
                <span key={d.id} className="px-2 py-0.5 rounded-full" style={{ backgroundColor: d.color }}>
                  {d.name}
                </span>
              ))}
            </div>
...
export default function Settings() {
  ...
  const archivedDoctors = state.doctors.filter(d => d.archived);

  return (
    <div className="max-w-2xl mx-auto space-y-8">
      {/* Personal rates - everyone */}
      <PersonalRateEditor />
        ...
        <>
          ...
          <section className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div key={d.id} className="flex items-center gap-3 py-2 border-b border-gray-100">
                  {editingDoctor === d.id ? (
                    ...
                    <>
                      ...
                      <button onClick={() => archiveDoctor(d.id)} className="text-xs text-yellow-600 hover:underline">보관</button>
                      <button
                        onClick={() => {
            ...
            {archivedDoctors.length > 0 && (
              <div className="mb-4">
                <p className="text-xs text-gray-400 mb-2">보관된 의사:</p>
                <div className="space-y-1">
                  {archivedDoctors.map(d => (
                    <div key={d.id} className="flex items-center gap-3 py-1.5 px-2 bg-gray-50 rounded">
                      <div className="w-4 h-4 rounded-full opacity-50" style={{ backgroundColor: d.color }} />
                      <span className="text-sm text-gray-400 flex-1">{d.name}</span>
                      <button onClick={() => restoreDoctor(d.id)} className="text-xs text-blue-600 hover:underline">복원</button>
                      <button
                        onClick={() => {
                          if (confirm(`${d.name}을(를) 완전 삭제하시겠습니까?`)) removeDoctor(d.id);
                        }}
                        className="text-xs text-red-500 hover:underline"
                      >삭제</button>
                    </div>
            ...
            <div className="flex gap-2">
              <input type="text" placeholder="새 의사 이름" value={newDoctorName}
          ...
          {/* Default rates */}
          <section className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="font-bold text-lg mb-4">기본 시급 설정 (지점)</h3>
            <p className="text-sm text-gray-500 mb-4">
              모든 의사에게 적용되는 기본 시급. 개인 설정이 있으면 개인 시급이 우선. (단위: 만원/시간)
            </p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
          ...
          <section className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">
              설/추석 연휴 등 기본 시급과 다른 시급이 적용되는 기간.
            </p>
              ...
              <div className="space-y-2 mb-4">
                {state.specialRatePeriods.map(p => (
                  <div key={p.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                    ...
                    <button onClick={() => removeSpecialRatePeriod(p.id)}
                      className="text-xs text-red-500 hover:underline ml-2">삭제</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <section className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={addCustomHoliday}
                className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">추가</button>
            </div>
          </section>
          ...
          <section className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => { if (confirm('모든 데이터를 초기화하시겠습니까?')) resetData(); }}
                className="bg-red-50 text-red-600 px-4 py-2 rounded-lg text-sm hover:bg-red-100">초기화</button>
            </div>
          </section>
        </>
      )}
